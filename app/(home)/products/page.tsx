...
const ProductsPage = () => {
  ...
  return (
    <section className="flex flex-1 flex-col justify-center w-full max-w-6xl mx-auto px-3 py-5 gap-10">
      <div className="w-full h-[250px] sm:h-[450px]  bg-gradient-to-r from-sky-200 to-sky-800 rounded-xl flex items-center justify-center">
      ...
      <div className="self-start">
        <DropdownMenu>
          <DropdownMenuTrigger>
            <Button>Sort by: {sortedType}</Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent>
            <DropdownMenuItem onClick={() => setSortedType("asc")}>
              ASC
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setSortedType("desc")}>
              DESC
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
