...
const CartPage = () => {
    ...
    const handleSubmitForm = async(event: FormEvent) => {
        ...
        setIsLoading(true)
        try {
            const {data} = await axios.post('/api/user/orders', {cart, ...input, totalPrice})
            console.log(data)
            setIsLoading(false)
            router.push('/')
            removeAll()
    ...
    return (
        <section className='max-w-6xl mx-auto w-full px-3 py-5'>
            {!cart.length ? (
                <span className='text-4xl font-bold h-screen flex items-center justify-center'>You don&apos;t have any items in cart</span>
            ) : (
                <div className='flex flex-col gap-10'>
                    {cart.length > 1 ? (
                        <div className='flex justify-between flex-wrap items-center gap-2'>
                            <span className='text-2xl font-bold'>Shopping Cart</span>
                            <Button onClick={removeAll} variant='destructive' className='flex items-center gap-2'>
                    ...
                    <div className="flex flex-col max-w-[400px] w-full self-center gap-3 bg-gray-100 rounded-lg p-5">
                        ...
                        <form className='flex flex-col gap-3' onSubmit={handleSubmitForm}>
                            <div>
                                ...
                                <Input id='fullName' value={input.fullName} onChange={handleChangeInput} required/>
                            </div>
                            <div>
                                <Label htmlFor='email'>Email</Label>
                                <Input id='email' value={input.email} onChange={handleChangeInput} required/>
                            </div>
                            <div>
                                <Label htmlFor='address'>Address</Label>
                                <Input id='address' value={input.address} onChange={handleChangeInput} required/>
                            </div>
